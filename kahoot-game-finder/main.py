...
def stream_scanner():
    ...
    for _ in range(max(1, WORKER_COUNT)):
        ...
        t.start()
        workers.append(t)
    print("✅ Scanner started with", len(workers), "workers.")
    try:
...
def flood():
    ...
    for i in range(amount):
        if random_names:
            ...
            bot_name = name
        # Real join logic would go here. This stub just returns info.
...
def check_pin():
    ...
    if not code.isdigit():
        return jsonify({"active": False, "message": "Invalid code!"})
    is_active, data = is_game_active(code)
    if is_active:
        msg = format_active_message(code, data)
        return jsonify({"active": True, "message": msg})
    else:
        return jsonify({"active": False, "message": f"Not active: {data}"})
